#!/bin/bash
# 过夜任务(GPU0): idf_clean_dis_v4 + L_ord，四档lambda_ord=0.0001/0.01/0.1/1.0，各10000步训练+评测。
# 被杀后每30分钟自动重试。
set -u
...
train_with_retry() {
    local LAMBDA_ORD=$1
    local MODEL_ID="idf_clean_dis_v4_ord${LAMBDA_ORD}_full10000"
    local CKPT_DIR="checkpoints/${MODEL_ID}"
    local CKPT="${CKPT_DIR}/${MODEL_ID}_final.pth"
    mkdir -p "$CKPT_DIR"
    local attempt=1
    while [ ! -f "$CKPT" ]; do
        echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 第${attempt}次训练尝试开始: $(date)"
        python3 pretrain.py \
            ...
            --tau_dis 0.17 --lambda_sem 0 --lambda_ord "$LAMBDA_ORD" --ord_margin 0.0 \
            --freeze_chronos_bolt --init_from_checkpoint "$V3_CKPT" --checkpoints "$CKPT_DIR" \
            > "logs/${MODEL_ID}_attempt${attempt}.log" 2>&1
        if [ -f "$CKPT" ]; then
            echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 训练成功(第${attempt}次): $(date)"
            break
        fi
        echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 第${attempt}次训练失败(无checkpoint)，${RETRY_WAIT}秒后重试: $(date)"
        attempt=$((attempt+1))
        sleep "$RETRY_WAIT"
    done
}
...
eval_with_retry() {
    ...
    local RESULT_FILE="results/forecast_evaluation/zeroshot_chronos_idf_clean_dis_eval_ord${LAMBDA_ORD}_full10000.txt"
    local attempt=1
    while true; do
        local n
        n=$(grep -c "^ETTh1\|^ETTh2\|^ETTm1\|^ETTm2\|^weather\|^exchange_rate" "$RESULT_FILE" 2>/dev/null || echo 0)
        if [ "$n" -eq 6 ]; then
            echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 评测已完整(6/6)，跳过: $(date)"
            break
        fi
        rm -f "$RESULT_FILE"
        echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 第${attempt}次评测尝试开始: $(date)"
        DATASETS="ETTh1 ETTh2 ETTm1 ETTm2 weather exchange_rate" \
            AUGMENT_MODE="idf_clean_dis_v4" \
            SAVE_SUFFIX="eval_ord${LAMBDA_ORD}_full10000" \
            CHECKPOINT_MODEL_PATH="$CKPT" \
            bash script/zeroshot_chronos_idf_clean_dis.sh \
            > "logs/zeroshot_v4_ord${LAMBDA_ORD}_full10000_attempt${attempt}.log" 2>&1
        n=$(grep -c "^ETTh1\|^ETTh2\|^ETTm1\|^ETTm2\|^weather\|^exchange_rate" "$RESULT_FILE" 2>/dev/null || echo 0)
        if [ "$n" -eq 6 ]; then
            echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 评测成功(第${attempt}次): $(date)"
            break
        fi
        echo "[overnight_gpu0_ord] lambda_ord=${LAMBDA_ORD} 第${attempt}次评测失败(得到${n}/6条)，${RETRY_WAIT}秒后重试: $(date)"
        attempt=$((attempt+1))
        sleep "$RETRY_WAIT"
    done
}

echo "==== overnight_gpu0_ord 开始: $(date) ===="
for LAMBDA_ORD in 0.0001 0.01 0.1 1.0; do
    train_with_retry "$LAMBDA_ORD"
    eval_with_retry "$LAMBDA_ORD"
done
echo "==== overnight_gpu0_ord 全部完成: $(date) ===="
