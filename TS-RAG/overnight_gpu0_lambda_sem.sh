#!/bin/bash
# 过夜任务(GPU0): lambda_sem=0.1, 1 各10000步训练+评测。
...
train_with_retry() {
    ...
    local attempt=1
    while [ ! -f "$CKPT" ]; do
        echo "[overnight_gpu0] lambda_sem=${LAMBDA} 第${attempt}次训练尝试开始: $(date)"
        python3 pretrain.py \
            --model_id "$MODEL_ID" \
        ...
        if [ -f "$CKPT" ]; then
            echo "[overnight_gpu0] lambda_sem=${LAMBDA} 训练成功(第${attempt}次): $(date)"
            break
        fi
        echo "[overnight_gpu0] lambda_sem=${LAMBDA} 第${attempt}次训练失败(无checkpoint，大概率被资源争抢杀掉)，${RETRY_WAIT}秒后重试: $(date)"
        attempt=$((attempt+1))
        sleep "$RETRY_WAIT"
    done
}
...
eval_with_retry() {
    ...
    local RESULT_FILE="results/forecast_evaluation/zeroshot_chronos_idf_clean_dis_eval_lambda${LAMBDA}_full10000.txt"
    local attempt=1
    while true; do
        local n
        n=$(grep -c "^ETTh1\|^ETTh2\|^ETTm1\|^ETTm2\|^weather\|^exchange_rate" "$RESULT_FILE" 2>/dev/null || echo 0)
        if [ "$n" -eq 6 ]; then
            echo "[overnight_gpu0] lambda_sem=${LAMBDA} 评测已完整(6/6)，跳过: $(date)"
            break
        fi
        rm -f "$RESULT_FILE"
        echo "[overnight_gpu0] lambda_sem=${LAMBDA} 第${attempt}次评测尝试开始: $(date)"
        DATASETS="ETTh1 ETTh2 ETTm1 ETTm2 weather exchange_rate" \
            ...
            > "logs/zeroshot_v4_lambda${LAMBDA}_full10000_attempt${attempt}.log" 2>&1
        n=$(grep -c "^ETTh1\|^ETTh2\|^ETTm1\|^ETTm2\|^weather\|^exchange_rate" "$RESULT_FILE" 2>/dev/null || echo 0)
        if [ "$n" -eq 6 ]; then
            echo "[overnight_gpu0] lambda_sem=${LAMBDA} 评测成功(第${attempt}次): $(date)"
            break
        fi
        echo "[overnight_gpu0] lambda_sem=${LAMBDA} 第${attempt}次评测失败(得到${n}/6条)，${RETRY_WAIT}秒后重试: $(date)"
        attempt=$((attempt+1))
        sleep "$RETRY_WAIT"
    done
}

echo "==== overnight_gpu0 开始: $(date) ===="
for L in 0.1 1; do
    train_with_retry "$L"
    eval_with_retry "$L"
done
echo "==== overnight_gpu0 全部完成: $(date) ===="
