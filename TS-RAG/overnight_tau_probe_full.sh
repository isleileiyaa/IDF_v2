#!/bin/bash
# 过夜任务: 固定lambda_ord=0.01/ord_margin=0.05/lambda_sem=0/lambda_xcov=0/tau_dis=0.17，
# 只扫--tau(Eq.19置信度温度)，测τ这一个新参数(由调用方通过TAU_VALUE/GPU_ID/TAG传入)。
...
RETRY_WAIT=1800
MODEL_ID="idf_clean_dis_v4_ord0.01_margin0.05_tau${TAU_VALUE}_full10000"
...
attempt=1
while [ ! -f "$CKPT" ]; do
    echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 第${attempt}次训练尝试开始: $(date)"
    python3 pretrain.py \
        ...
        --tau_dis 0.17 --lambda_sem 0 --lambda_ord 0.01 --ord_margin 0.05 --tau "$TAU_VALUE" \
        --freeze_chronos_bolt --init_from_checkpoint "$V3_CKPT" --checkpoints "$CKPT_DIR" \
        > "logs/${MODEL_ID}_attempt${attempt}.log" 2>&1
    if [ -f "$CKPT" ]; then
        echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 训练成功(第${attempt}次): $(date)"
        break
    fi
    echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 第${attempt}次训练失败(无checkpoint)，${RETRY_WAIT}秒后重试: $(date)"
    attempt=$((attempt+1))
    sleep "$RETRY_WAIT"
done

RESULT_FILE="results/forecast_evaluation/zeroshot_chronos_idf_clean_dis_eval_ord0.01_margin0.05_tau${TAU_VALUE}_full10000.txt"
attempt=1
while true; do
    n=$(grep -c "^ETTh1\|^ETTh2\|^ETTm1\|^ETTm2\|^weather\|^exchange_rate" "$RESULT_FILE" 2>/dev/null || echo 0)
    if [ "$n" -eq 6 ]; then
        echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 评测已完整(6/6)，跳过: $(date)"
        break
    fi
    rm -f "$RESULT_FILE"
    echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 第${attempt}次评测尝试开始: $(date)"
    DATASETS="ETTh1 ETTh2 ETTm1 ETTm2 weather exchange_rate" \
        AUGMENT_MODE="idf_clean_dis_v4" \
        SAVE_SUFFIX="eval_ord0.01_margin0.05_tau${TAU_VALUE}_full10000" \
        CHECKPOINT_MODEL_PATH="$CKPT" \
        bash script/zeroshot_chronos_idf_clean_dis.sh \
        > "logs/zeroshot_v4_ord0.01_margin0.05_tau${TAU_VALUE}_full10000_attempt${attempt}.log" 2>&1
    n=$(grep -c "^ETTh1\|^ETTh2\|^ETTm1\|^ETTm2\|^weather\|^exchange_rate" "$RESULT_FILE" 2>/dev/null || echo 0)
    if [ "$n" -eq 6 ]; then
        echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 评测成功(第${attempt}次): $(date)"
        break
    fi
    echo "[overnight_tau_probe_${TAG}] tau=${TAU_VALUE} 第${attempt}次评测失败(得到${n}/6条)，${RETRY_WAIT}秒后重试: $(date)"
    attempt=$((attempt+1))
    sleep "$RETRY_WAIT"
done
echo "==== overnight_tau_probe_${TAG}(tau=${TAU_VALUE}) 全部完成: $(date) ===="
