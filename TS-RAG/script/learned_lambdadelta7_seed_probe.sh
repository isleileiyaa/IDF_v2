...
export PRETRAIN_SEED="${PROBE_SEED:-1234}"
...
lambda_delta=7.0
model_id="idf_clean_dis_v4_fusion_learned_lambdadelta7.0_fromscratch_full10000_seedprobe${PRETRAIN_SEED}"
...
mkdir -p "$checkpoints"
echo "Launching pretrain (fromscratch, no init_from_checkpoint) SEED PROBE with PRETRAIN_SEED=$PRETRAIN_SEED, lambda_delta=$lambda_delta (noise-floor check vs original seed=2021 run)"
