cd /home/fenglei/TS-RAG-main/TS-RAG   # 换成你本地实际路径

export PRETRAIN_SEED=2021   # !! 换成你训 idf_clean_dis 正式模型时用的种子，要保持一致 !!
...
augment_mode=idf_clean_dis          # 架构、冻结列表跟真实RAG模型完全一致
context_length=512
...
model_id="data50m_${augment_mode}_baseablation_${context_length}_pred${prediction_length}_lookback${retrieve_lookback_length}_top${top_k}_lr${lr}_drop${drop_prob}_${optimizer}_cosanneal_step${train_steps}_bs${batch_size}_no_embeddingtuning_seed${PRETRAIN_SEED}"
echo "Launching Base-arm ablation (kill_retrieval), model_id=$model_id"
