...
# augment_mode has no effect on Moirai2Retrieve model construction (its RIDDE
# head is a single fixed point-forecast architecture, see models/Moirai2.py)
# -- kept here only so --freeze_chronos_bolt's layer-name-matching logic
# below has a value to key off of; the new-head layer names happen to match
# the idf_clean_dis set, so this is harmless either way.
