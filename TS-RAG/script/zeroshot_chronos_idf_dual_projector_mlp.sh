...
filename="${SAVE_FILE_NAME:-zeroshot_chronos_idf_dual_projector_mlp${save_suffix:+_${save_suffix}}.txt}"
...
lookback_length=512
augment_mode=idf_dual_projector_mlp
top_k=10
tau="${TAU:-0.1}"
...
checkpoint_model_path="${CHECKPOINT_MODEL_PATH:-/home/fenglei/TS-RAG-main/TS-RAG/checkpoints/data50m_idf_dual_projector_mlp_512_pred64_lookback512_top10_lr0.0003_drop0.2_adamw_cosanneal_step10000_bs256_no_embeddingtuning_lambda0.001_10_tau0.1_final.pth}"
...
python $run_file \
    ...
    --model_id "${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_idf_dual_projector_mlp" \
    --data $data \
    --top_k $top_k \
