...
filename="${SAVE_FILE_NAME:-zeroshot_timesfm25_idf_clean_dis${save_suffix:+_${save_suffix}}.txt}"
...
# NOTE: TimesFM25ModelForForecastingWithRetrieval does not read
# --pretrained_model_path (its backbone is always
# google/timesfm-2.5-200m-pytorch, loaded via the native timesfm package
# inside the model class itself) -- no pretrained_model_path arg is passed below.
default_checkpoint_model_path="/home/fenglei/TS-RAG-main/TS-RAG/checkpoints/timesfm25_idf_clean_dis_512_pred64_lookback512_top10_lr0.000005_drop0.2_adamw_cosanneal_step10000_bs256_final.pth"
...
python $run_file \
    ...
    --model_id "${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_timesfm25_idf_clean_dis" \
    --data $data \
    --top_k $top_k \
