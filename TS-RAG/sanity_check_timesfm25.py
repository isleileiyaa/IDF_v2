"""
Sanity check for models/TimesFM25.py -- NOT a training script.

Builds a small fake batch, runs one full forward pass through
TimesFM25ModelForForecastingWithRetrieval, and prints the shape of every
...
frozen/trainable parameter breakdown. Mirrors sanity_check_moirai2.py.
"""

import torch

from models.TimesFM25 import RIDDE_HEAD_LAYER_NAMES, TimesFM25ModelForForecastingWithRetrieval
...
def main():
    ...
    # --- frozen/trainable parameter breakdown ---
    trainable, frozen = [], []
    ...
    for expected in RIDDE_HEAD_LAYER_NAMES:
        assert any(name.startswith(expected) for name, _ in trainable), (
            f"expected head '{expected}' not found among trainable params"
        )
    print("OK: only the new RIDDE head layers are trainable; backbone is fully frozen.")
    ...
    # --- backward pass: confirm gradients reach only the new heads ---
    out.loss.backward()
